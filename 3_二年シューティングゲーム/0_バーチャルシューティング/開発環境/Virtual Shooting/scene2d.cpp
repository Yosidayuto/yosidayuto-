...
HRESULT CScene2d::Init(void)
{
	...
	////距離計算
	m_fLengthpos = sqrtf(powf(m_sizePolygon.x / 2.0f, 2.0f) + powf(m_sizePolygon.y / 2.0f, 2.0f));
	m_fAngle = atan2f(m_sizePolygon.x / 2.0f, m_sizePolygon.y / 2.0f);
...
{
	VERTEX_2D *pVtx;//頂点情報へのポインター
	float fAngleX = m_fLengthpos*cosf(m_fAngle);
	float fAngleY = m_fLengthpos*sinf(m_fAngle);

	m_pVtxBuff->Lock(0, 0, (void**)&pVtx, 0);
	// 頂点バッファの移動
	pVtx[0].pos = D3DXVECTOR3(m_posPolygon.x + fAngleX, m_posPolygon.y + fAngleY, 0.0f);
	fAngleX = m_fLengthpos*cosf(m_fAngle + D3DXToRadian(90));
	fAngleY = m_fLengthpos*sinf(m_fAngle + D3DXToRadian(90));
	...
	fAngleX = m_fLengthpos*cosf(m_fAngle + D3DXToRadian(180));
	fAngleY = m_fLengthpos*sinf(m_fAngle + D3DXToRadian(180));
