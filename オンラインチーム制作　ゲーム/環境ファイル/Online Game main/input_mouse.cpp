...
HRESULT CInhMouse::Init(HINSTANCE hInstance, HWND hWnd)
{
...
void CInhMouse::Uninit(void)
{
	CInput::Uninit();
}


void CInhMouse::Update(void)
{
...
#else 
POINT CInhMouse::GetMousePos(void)
{
	return posMouse;
}
#endif

bool CInhMouse::GetClick(int nClick)
{
	return (m_aClickState.rgbButtons[nClick] & 0x80) ? true : false;
}

bool CInhMouse::GetClickTrigger(int nClick)
{
	return (m_aClickTrigger.rgbButtons[nClick] & 0x80) ? true : false;
}

bool CInhMouse::GetClickRelease(int nClick)
{
