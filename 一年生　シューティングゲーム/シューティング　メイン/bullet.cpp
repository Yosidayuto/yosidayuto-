...
void InitBullet(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, "bullet000.png", &g_pTextureBullet);//テクスチャの読み込み
	...
	for (nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		if (g_aBullet[nCntBullet].bUse == true)//弾が使用されている					
		{
			...
			{/*プレイヤーの弾*/
				...
				SetEffect(g_aBullet[nCntBullet].pos, D3DXCOLOR(255,255 , 0, 10), 1.5f, 6);
			...
			{/*敵の弾*/
				SetEffect(g_aBullet[nCntBullet].pos, D3DXCOLOR(255,0, 0, 10), 1.5f, 5);
				pPlayer = GatPlayer();
