...
void InitEnemy(void)
{
	...
	/*テクスチャの読み込み*/
	D3DXCreateTextureFromFile(pDevice, "enemy000.png", &g_apTextureEnemy[0]);//テクスチャの読み込み
		...
		if (g_aEnemy[nCntEnemy].bUse == true)//敵が使用されている
		{
			...
			case ENEMYSTATE_NORMAL://通常時
				...
				if ((rand() % (g_nCountEnemy * 20)) == 0)
				{
					...
					SetBullet(g_aEnemy[nCntEnemy].pos, D3DXVECTOR3(sinf(fAngle)*8, cosf(fAngle)*8, 0.0f), BULLETTYPE_ENEMY, 100, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));//敵の攻撃
				}
				break;
