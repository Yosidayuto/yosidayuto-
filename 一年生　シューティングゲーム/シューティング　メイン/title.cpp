...
void InintTitle(void)
{
	...
	//テクスチャの読み込み	
	D3DXCreateTextureFromFile(pDevice, "bg000.png", &g_pTextureTitle);
	...
	D3DXCreateTextureFromFile(pDevice, "title_logo.png", &g_pTextureTitlerogo);
	pDevice->CreateVertexBuffer(sizeof(VERTEX_2D)*NUM_VERTEX *3,//バッファサイズ
		D3DUSAGE_WRITEONLY,//(固定)
		FVF_VERTEX_2D,//頂点フォーマット
		D3DPOOL_MANAGED,//（固定）
	...
	D3DXCreateTextureFromFile(pDevice, "press_enter.png", &g_pTextureTitlePusy);
	pDevice->CreateVertexBuffer(sizeof(VERTEX_2D)*NUM_VERTEX * 3,//バッファサイズ
		D3DUSAGE_WRITEONLY,//(固定)
		FVF_VERTEX_2D,//頂点フォーマット
		D3DPOOL_MANAGED,//（固定）
