...
void UpdateBoos(void) 
{
	...
	/*振り向き処理*/
	g_Boos.rot += (g_Boos.rotDest - g_Boos.rot)* 0.09f;	
	/*慣性*/
...
void UpdateAnimation()
{
	if (g_frameCount == 0)
	{
		...
		{
			//アニメーションアップデート
			g_Boos.modelParts[nAnime].rotAnime =(g_motion[g_boosCurrentAnimation].keySet[g_playKeySetlndex].key[nAnime].rot-g_Boos.modelParts[nAnime].rot) / (float)g_motion[g_boosCurrentAnimation].keySet[g_playKeySetlndex].frame;
			g_Boos.modelParts[nAnime].posAnime = (g_motion[g_boosCurrentAnimation].keySet[g_playKeySetlndex].key[nAnime].pos - g_Boos.modelParts[nAnime].pos) / (float)g_motion[g_boosCurrentAnimation].keySet[g_playKeySetlndex].frame;
		}
	}
	//アニメーションアップデート
	for (int nAnime = 0; nAnime < BOOS_PARTS_MAX; nAnime++)
