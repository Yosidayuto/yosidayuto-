...
void InitEnemy(void)
{
	...
	for (int nEnemy = 0; nEnemy < MAX_ENEMY; nEnemy++)
	{
		for (int i = 0; i < ENEMY_PARTS_MAX; i++)
		{
			/*Xファイルの読み込み*/
			D3DXLoadMeshFromX(
				g_FileEnemy[i].pFilename,
				D3DXMESH_MANAGED,
				pDevice,
				NULL,
				&g_Enemy[nEnemy].enemyParts[i].pBuffMat,
				NULL,
				&g_Enemy[nEnemy].enemyParts[i].nNumMat,
				&g_Enemy[nEnemy].enemyParts[i].pMesh);
			/*マテリアルデータへのポインタを取得*/
			pMatEnemy[i] = (D3DXMATERIAL*)g_Enemy[nEnemy].enemyParts[i].pBuffMat->GetBufferPointer();
			/*ファイルの読み込み*/
			for (int nCntMat = 0; nCntMat < (int)g_Enemy[nEnemy].enemyParts[i].nNumMat; nCntMat++)
			{
				if (pMatEnemy[i][nCntMat].pTextureFilename != NULL)
				{
					D3DXCreateTextureFromFile(pDevice, pMatEnemy[i][nCntMat].pTextureFilename, &g_Enemy[nEnemy].enemyParts[i].pTextureModel[nCntMat]);
				}
			}
		}
	}
