...
HRESULT CResult::Init(void)
{
	...
	for (int nCount = 0; nCount < MAX_RANKING; nCount++)
	{
		if (nCount<=4)
		{
			m_pNumber[nCount] = CNumber::Create(D3DXVECTOR3(180, 235 + (nCount * 100), 0.0f), D3DXVECTOR3(40, 80, 0.0f));
			m_Score[nCount] = CScore::Create(D3DXVECTOR3(220,200+(nCount*100),0.0f), D3DXVECTOR3(40,80,0.0f), false);
		}
		else
		{
			m_pNumber[nCount] = CNumber::Create(D3DXVECTOR3(700, 235 + ((nCount-5) * 100), 0.0f), D3DXVECTOR3(40, 80, 0.0f));
			m_Score[nCount] = CScore::Create(D3DXVECTOR3(740, 200 + ((nCount-5) * 100), 0.0f), D3DXVECTOR3(40, 80, 0.0f), false);
		}
...
void CResult::ReadFile(CGame::STAGE_TYPE Stage)
{
	...
	pFile = fopen(pTexture[Stage], "r");	//テキストファイル読み込み
	if (pFile != NULL)
	{
		for (int nCount = 0; nCount < MAX_RANKING; nCount++)
		{
			fscanf(pFile, "%d", &nScoreData[nCount]);
		}
		fclose(pFile);
	}
...
void CResult::WriteFile(CGame::STAGE_TYPE Stage)
{
	...
	pFile = fopen(pTexture[Stage], "w");	//テキストファイル書き込み
