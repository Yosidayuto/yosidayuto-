...
void InitEffect(void)
{
	...
	for (nCntEffect = 0; nCntEffect< MAX_EFFECT; nCntEffect++)
	{
		...
		g_aEffect[nCntEffect].fColAlphaValue = 255;
	}
	D3DXCreateTextureFromFile(pDevice, "TEXTURE/bullet.png", &g_pTextureEffect);//テクスチャの読み込み
...
void UpdateEffect(void)
{
	...
	for (nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		if (g_aEffect[nCntEffect].bUse == true)
		{
			...
			g_aEffect[nCntEffect].fColAlphaValue++;
			...
			if (g_aEffect[nCntEffect].fColAlphaValue >= 1)
			{
				g_aEffect[nCntEffect].fColAlphaValue = 0;
				g_aEffect[nCntEffect].col= D3DXCOLOR(g_aEffect[nCntEffect].col.r, g_aEffect[nCntEffect].col.g, g_aEffect[nCntEffect].col.b, g_aEffect[nCntEffect].col.a-0.1f);
			}
