...
void InitCamera(void)
{
	...
	g_camera.fmove = 0;
	/*カメラの位置・注視点・上方向を設定*/
	g_camera.posV = D3DXVECTOR3(0.0f, 40.0f, g_camera.fDistance);
...
void UpdateCamera(void)
{
	...
	{ // 成功
		if (JOYPAD.dwRpos > 0x7FFF + 0x4000)
		{
			g_camera.rot.x += 1.0f;
			g_camera.posV.y = g_camera.posR.y + sinf(D3DXToRadian(g_camera.rot.x)) * g_camera.fDistance;
			g_camera.posV.z = g_camera.posR.z + cosf(D3DXToRadian(g_camera.rot.x)) * g_camera.fDistance;
			/*g_camera.posV.y = g_camera.posR.y + tanf(D3DXToRadian(g_camera.rot.x)) * g_camera.fDistance;
		...
		if (JOYPAD.dwUpos > 0x7FFF + 0x4000)
		{
			g_camera.rot.y += 1.0f;
